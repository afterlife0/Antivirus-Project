"""
XGBoost Model Implementation for EMBER2018 Malware Detection
Independent robust XGBoost implementation with hyperparameter tuning

FILE CONNECTION MAP
==================
Dependencies (files this module imports from):
- None (independent model using XGBoost only)

Connected Components (files that import from this module):
- trainer.py (imports XGBoostModel class)

Integration Points:
- Provides XGBoost model implementation for malware detection
...
□ Class name matches exactly: XGBoostModel
...
try:
    ...
    print("❌ CRITICAL ERROR: XGBoost not available")
...
XGBOOST_HYPEROPT_SPACE = {
    ...
    'gamma': hp.loguniform('gamma', np.log(0.01), np.log(2))
}
...
class XGBoostModel:
    ...
    def __init__(self, random_state: int = 42, n_cores: int = -1, memory_limit: float = 8.0):
        ...
        self.default_param_grid = XGBOOST_PARAM_GRID
        self.fast_param_grid = XGBOOST_PARAM_GRID_FAST
    ...
    def _create_base_model(self, **params) -> XGBClassifier:
        """
        Create base XGBoost model with specified parameters
        **FIXED**: Maximum compatibility with all XGBoost versions
        ...
        """
        try:
            # **COMPATIBILITY**: Start with minimal parameters that work everywhere
            base_params = {
                'random_state': self.random_state,
                'n_jobs': self.n_cores if self.n_cores > 0 else 1,
                'verbosity': 0
            }
            
            # Add standard XGBoost parameters
            standard_params = {
                'n_estimators': 100,
                'max_depth': 6,
                'learning_rate': 0.1,
            ...
            # **VERSION-SPECIFIC PARAMETERS**: Add only if supported
            try:
                # Test if objective parameter is supported
                test_model = XGBClassifier(n_estimators=1, verbosity=0)
                if hasattr(test_model, 'objective'):
                    standard_params['objective'] = 'binary:logistic'
                    
                # Test if eval_metric is supported
                if hasattr(test_model, 'eval_metric'):
                    standard_params['eval_metric'] = 'logloss'
                    
                # Test if enable_categorical is supported (newer versions)
                try:
                    XGBClassifier(enable_categorical=False, n_estimators=1, verbosity=0)
                    standard_params['enable_categorical'] = False
                except Exception:
                    pass  # Skip if not supported
                    
            except Exception:
                print("⚠️ Using minimal XGBoost parameters for compatibility")
            
            # Combine all parameters
            model_params = {**base_params, **standard_params, **params}
            
            # **CRITICAL**: Remove any parameters that might cause issues
            problematic_params = [
                'early_stopping_rounds', 'callbacks', 'eval_set', 
                'validation_fraction', 'validation_data'
            ]
            
            for param in problematic_params:
                if param in model_params:
                    removed_value = model_params.pop(param)
                    print(f"🔧 Removed problematic parameter: {param} = {removed_value}")
            
            # **SAFE CREATION**: Try creating model with error handling
            try:
                print(f"🔧 Creating XGBoost model with parameters: {list(model_params.keys())}")
                model = XGBClassifier(**model_params)
                print("✅ XGBoost model created successfully")
                return model
                
            except Exception as creation_error:
                print(f"⚠️ Parameter-rich model creation failed: {creation_error}")
                
                # **FALLBACK**: Create with only essential parameters
                essential_params = {
                ...
                print(f"🔧 Trying essential parameters only: {essential_params}")
                model = XGBClassifier(**essential_params)
                print("✅ Essential XGBoost model created successfully")
                return model
            
        except Exception as e:
            print(f"❌ Error creating XGBoost model: {e}")
            print(f"🔧 Creating absolute minimal XGBoost model...")
            
            # **ABSOLUTE FALLBACK**: Most basic model possible
            try:
                model = XGBClassifier(n_estimators=100, random_state=self.random_state)
                print("✅ Minimal XGBoost model created")
                return model
            except Exception as minimal_error:
                print(f"❌ Even minimal model creation failed: {minimal_error}")
                raise
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series, 
        ...
        **FIXED**: Proper early stopping and fit() method parameter handling
        ...
        try:
            ...
            # **FIXED**: Handle early stopping properly
            early_stopping_rounds = config.get('early_stopping_rounds', 50)
            
            # Add early stopping to model parameters if validation data is available
            if X_val_array is not None and y_val_array is not None and early_stopping_rounds:
            ...
            # **FIXED**: Robust fit() method that works with all XGBoost versions
            try:
                # **STEP 1**: Determine XGBoost capabilities
                xgb_version_str = xgb.__version__
                print(f"🔍 XGBoost version detected: {xgb_version_str}")
                
                # **STEP 2**: Check if validation data is available
                has_validation = X_val_array is not None and y_val_array is not None
                wants_early_stopping = early_stopping_rounds and early_stopping_rounds > 0
                
                if has_validation and wants_early_stopping:
                    print(f"🛑 Attempting training with early stopping ({early_stopping_rounds} rounds)...")
                    eval_set = [(X_train_array, y_train_array), (X_val_array, y_val_array)]
                    
                    # **TRY 1**: Modern XGBoost with callbacks
                    try:
                        ...
                        print("🔄 Trying callback-based early stopping...")
                        self.model.fit(
                        ...
                        print("✅ Callback-based early stopping successful")
                        
                    except Exception as callback_error:
                        print(f"⚠️ Callback early stopping failed ({type(callback_error).__name__})")
                        
                        # **TRY 2**: Parameter-based early stopping
                        try:
                            print("🔄 Trying parameter-based early stopping...")
                            self.model.fit(
                                X_train_array, y_train_array,
                                eval_set=eval_set,
                            ...
                            print("✅ Parameter-based early stopping successful")
                            
                        except Exception as param_error:
                            print(f"⚠️ Parameter early stopping failed ({type(param_error).__name__})")
                            
                            # **TRY 3**: Training with eval_set but no early stopping
                            try:
                                print("🔄 Trying validation monitoring without early stopping...")
                                self.model.fit(
                                    X_train_array, y_train_array,
                                    eval_set=eval_set,
                                    verbose=config.get('verbose', False)
                                )
                                print("✅ Validation monitoring successful (no early stopping)")
                                
                            except Exception as eval_error:
                                print(f"⚠️ Eval set training failed ({type(eval_error).__name__})")
                                
                                # **TRY 4**: Simple training without validation
                                print("🔄 Falling back to simple training...")
                                self.model.fit(X_train_array, y_train_array)
                                print("✅ Simple training successful")
                
                elif has_validation:
                    # Validation data available but no early stopping requested
                    eval_set = [(X_train_array, y_train_array), (X_val_array, y_val_array)]
                    ...
                    try:
                        ...
                        print("✅ Validation monitoring successful")
                        
                    except Exception as eval_error:
                        print(f"⚠️ Validation monitoring failed ({type(eval_error).__name__})")
                        print("🔄 Falling back to simple training...")
                        self.model.fit(X_train_array, y_train_array)
                        print("✅ Simple training successful")
                
                else:
                    # No validation data - simple training
                    print("🏋️ Training without validation data...")
                    self.model.fit(X_train_array, y_train_array)
                    print("✅ Simple training successful")
                    
            except Exception as final_error:
                print(f"❌ All training methods failed: {final_error}")
                print("🔄 Last resort: Minimal training...")
                
                # **FINAL FALLBACK**: Most basic training possible
                try:
                    # Create a new model with minimal parameters to ensure compatibility
                    minimal_params = {
                    ...
                    self.model = XGBClassifier(**minimal_params)
                    self.model.fit(X_train_array, y_train_array)
                    print("✅ Minimal training successful")
                    
                except Exception as minimal_error:
                    print(f"❌ Even minimal training failed: {minimal_error}")
                    raise RuntimeError(f"XGBoost training completely failed. Version: {xgb_version_str}")
        ...
        try:
            ...
            if method == 'grid':
                ...
                def objective(trial):
                    # Sample parameters
                    params = {}
                    for param, config in XGBOOST_OPTUNA_SPACE.items():
...
if __name__ == "__main__":
    print("🚀 XGBoost Model - Standalone Testing")
    print("=" * 60)
    
    try:
        # Initialize model
        xgb_model = XGBoostModel(random_state=42, n_cores=4, memory_limit=8.0)
        ...
        X_train = pd.DataFrame(X_sample, columns=feature_names)
        y_train = pd.Series(y_sample, name='target')
        
        print(f"   Sample data created: {X_train.shape[0]} samples, {X_train.shape[1]} features")
        
        # Train model on sample data
        print("\n🚀 Training XGBoost model on sample data...")
        train_results = xgb_model.train(X_train, y_train, use_hyperparameter_tuning=True)
        
        print(f"✅ Model trained successfully")
        print(f"   Best parameters: {train_results['best_parameters']}")
        print(f"   Training accuracy: {train_results['train_metrics']['accuracy']:.4f}")
        
        # Evaluate model
        print("\n📈 Evaluating model performance...")
        eval_results = xgb_model.evaluate(X_train, y_train)
        
        print(f"✅ Evaluation completed")
        for metric, value in eval_results.items():
            if isinstance(value, list) and len(value) > 0 and isinstance(value[0], float):
                # Print average of curve data
                print(f"   {metric}: {np.mean(value):.4f} (sampled)")
            else:
        ...
        save_path = "outputs/models/test_xgboost_model"
        Path("outputs/models").mkdir(parents=True, exist_ok=True)
        
        if xgb_model.save_model(save_path):
        ...
        xgb_tuning_model = XGBoostModel(random_state=42, n_cores=2, memory_limit=8.0)
        
        tuning_results = xgb_tuning_model.train(
            X_train, y_train,
        ...
        cv_results = xgb_model.cross_validate(X_train, y_train, cv_folds=3)
        ...
        if xgb_tuning_model.hyperparameter_tuning_results:
            print("\n💾 Testing hyperparameter results saving...")
            hp_save_path = "outputs/hyperparameter_results/test_xgboost_hyperparams"
            Path("outputs/hyperparameter_results").mkdir(parents=True, exist_ok=True)
            
            if xgb_tuning_model.save_hyperparameter_results(hp_save_path):
    ...
    print("\n🚀 XGBoost Model testing completed.")